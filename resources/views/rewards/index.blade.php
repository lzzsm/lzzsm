...
@section('title', 'Gerenciar Recompensas')
...
@section('content')
    <div class="bg-slate-900 text-gray-200 min-h-screen">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="mb-8 text-center">
                <h1 class="text-4xl font-extrabold text-white tracking-tight">Gerenciamento de Recompensas</h1>
                <p class="mt-2 text-lg text-gray-400">Modere, filtre e administre as recompensas da plataforma.</p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-x-6 gap-y-4 items-end">
                        ...
                        <div class="md:col-span-7">
                            ...
                            <div class="relative">
                                ...
                                <a href="{{ route('rewards.index', ['fields' => request('fields')]) }}"
                    ...
                    @foreach ($rewards as $reward)
                        <div
                            ...
                            <div class="bg-slate-900/50 px-6 py-4 flex items-center justify-end space-x-3">
                                ...
                                <button type="button"
                                    onclick="confirmarExclusaoRecompensa('{{ $reward->id }}', '{{ addslashes($reward->titulo) }}', '{{ $reward->pontos_necessarios }}', '{{ $reward->qtd_disponivel }}', '{{ $reward->empresa && $reward->empresa->user ? addslashes($reward->empresa->user->name) : 'Empresa não encontrada' }}')"
    ...
    <script>
        // Função para confirmar exclusão de recompensa
        function confirmarExclusaoRecompensa(rewardId, rewardTitulo, rewardPontos, rewardQuantidade, empresaNome) {
            Swal.fire({
                html: `
            <div class="text-center">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-gray-400 text-sm mt-1">Empresa: ${empresaNome}</p>
                </div>
                
                <p class="text-gray-300 mb-6 mt-6">Tem certeza que deseja excluir a recompensa <strong class="text-white">"${rewardTitulo}"</strong>?</p>
            </div>
        `,
