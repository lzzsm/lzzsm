...
@section('title', 'Criar Nova Recompensa')
    ...
    <div class="relative min-h-screen flex ">
        ...
        <div
            ...
            <div class="sm:w-1/2 xl:w-2/5 h-full hidden md:flex flex-auto items-center justify-start p-10 overflow-hidden text-white bg-no-repeat bg-cover relative"
                ...
                <div class="absolute triangle max-h-screen right-0 w-16 bg-gradient-to-b from-emerald-700 to-slate-950"></div>
                <img src="{{ asset('img/reciclagem.png') }}" class="h-96 absolute right-1">
                <div class="w-full max-w-md z-10">
                    <div class="sm:text-4xl xl:text-5xl font-bold leading-tight mb-6">Crie suas recompensas</div>
                    <div class="sm:text-sm xl:text-md text-gray-200 font-normal">Ofereça benefícios aos usuários em troca de pontos. É uma forma de engajar ainda mais sua empresa na comunidade.</div>
                </div>
                <ul class="circles">
            ...
            <div
                ...
                <div class="max-w-md w-full space-y-8">
                    <div class="text-center">
                        <h2 class="mt-6 text-3xl font-bold text-lime-400">Criar Recompensa</h2>
                        <p class="mt-2 text-sm text-slate-300">Preencha os campos para cadastrar sua recompensa.</p>
                    </div>

                    <form class="mt-8 space-y-6" method="POST" action="{{ route('rewards.store') }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div>
                            ...
                            <div class="relative mt-1">
                                ...
                                <input id="titulo" name="titulo" type="text" required
                                    class="w-full text-base pl-10 pr-4 py-2 border-b rounded-2xl focus:outline-none focus:border-green-500 @error('titulo') border-red-500 @else border-emerald-900 @enderror"
                                    placeholder="Ex: Desconto de 10% em compras" value="{{ old('titulo') }}">
                            </div>
                            @error('titulo')
                        ...
                        <!-- Descrição -->
                        <div>
                            <label for="descricao" class="ml-3 text-sm font-bold text-green-400 tracking-wide">Descrição</label>
                            <textarea id="descricao" name="descricao" rows="3"
                                class="w-full text-base px-4 py-2 border-b rounded-2xl focus:outline-none focus:border-green-500 border-emerald-900 @error('descricao') border-red-500 @enderror"
                                placeholder="Detalhes sobre a recompensa...">{{ old('descricao') }}</textarea>
                            @error('descricao')
                                <span class="ml-3 text-xs text-red-400">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <!-- Pontos Necessários -->
                        <div>
                            <label for="pontos_necessarios" class="ml-3 text-sm font-bold text-green-400 tracking-wide">Pontos Necessários</label>
                            <input id="pontos_necessarios" name="pontos_necessarios" type="number" required
                                class="w-full text-base px-4 py-2 border-b rounded-2xl focus:outline-none focus:border-green-500 border-emerald-900 @error('pontos_necessarios') border-red-500 @enderror"
                                placeholder="Ex: 200" value="{{ old('pontos_necessarios') }}">
                        ...
                        <div>
                            <label for="qtd_disponivel" class="ml-3 text-sm font-bold text-green-400 tracking-wide">Quantidade Disponível</label>
                            <input id="qtd_disponivel" name="qtd_disponivel" type="number" required
                                class="w-full text-base px-4 py-2 border-b rounded-2xl focus:outline-none focus:border-green-500 border-emerald-900 @error('qtd_disponivel') border-red-500 @enderror"
                                placeholder="Ex: 50" value="{{ old('qtd_disponivel') }}">
                        ...
                        <!-- Imagem -->
                        <div>
                            <label for="img_recompensa" class="ml-3 text-sm font-bold text-green-400 tracking-wide">Imagem da Recompensa (Opcional)</label>
                        ...
                        <span class="flex flex-col items-center justify-center mt-10 text-center text-md text-gray-500">
                            <p>Quer ver suas recompensas já cadastradas?</p>
