...
@section('content')
    <div class="bg-slate-900 text-gray-200 min-h-screen">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="mb-8 text-center">
                <h1 class="text-4xl font-extrabold text-white tracking-tight">Gerenciamento de Empresas Parceiras</h1>
                <p class="mt-2 text-lg text-gray-400">Visualize, filtre e administre as empresas parceiras da plataforma.
                </p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-x-6 gap-y-4 items-end">
                        ...
                        <div class="md:col-span-7">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="search"
                                    ...
                                    placeholder="Pesquisar por nome, email, CNPJ..." value="{{ request('search') }}">

                                <!-- Botão X SEMPRE visível, mas inativo quando não tem busca -->
                                <a href="{{ route('empresas.index', ['fields' => request('fields')]) }}"
                        ...
                        <div class="md:col-span-3">
                            ...
                            <div class="flex flex-wrap gap-3 pt-2">
                                ...
                                <x-custom-checkbox id="field_cnpj" name="fields[]" value="cnpj" label="CNPJ"
                                    :checked="in_array('cnpj', request('fields', []))" />
                                <x-custom-checkbox id="field_telefone" name="fields[]" value="telefone" label="Telefone"
                                    :checked="in_array('telefone', request('fields', []))" />
                            </div>
                        </div>
            ...
            @if ($empresas->isEmpty())
                <div class="text-center py-16 px-4 bg-gray-800/50 rounded-lg shadow-md border border-gray-700">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-200">Nenhuma empresa encontrada</h3>
                    <p class="mt-1 text-sm text-gray-400">Tente ajustar sua busca ou filtros, ou cadastre uma nova empresa.
                    </p>
                    <div class="mt-6">
                        <a href="{{ route('empresas.create') }}"
                            ...
                            <span class="text-base font-medium">Cadastrar Nova Empresa</span>
                        </a>
                    </div>
                </div>
            @else
                    ...
                    @foreach ($empresas as $empresa)
                        <div
                                    ...
                                    <div class="flex-shrink-0">
                                        ...
                                        @else
                                            <div
                                                ...
                                                <span
                                                    class="text-2xl font-bold text-lime-300">{{ strtoupper(substr($empresa->user->name, 0, 1)) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ route('empresas.show', $empresa) }}"
                                            class="text-lg font-semibold text-white truncate hover:text-lime-300 transition-colors">{{ $empresa->user->name }}</a>
                                        <p class="text-sm text-gray-400 truncate">{{ $empresa->user->email }}</p>
                                    </div>
                                </div>
                                    ...
                                    <dl class="space-y-3">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <dd class="text-sm text-gray-300 font-mono">
                                                {{ $empresa->cnpj_formatado ?? 'N/A' }}</dd>
                                        </div>
                                        <div class="flex items-center justify-between">
                                            <dt class="text-sm font-medium text-gray-500">Telefone</dt>
                                            <dd class="text-sm text-gray-300 font-mono">
                                                {{ $empresa->telefone_comercial_formatado ?? 'N/A' }}</dd>
                                        </div>
                                        <div class="flex items-center justify-between">
                                            <dt class="text-sm font-medium text-gray-500">Site</dt>
                                            <dd class="text-sm text-gray-300">
                                                <a href="{{ $empresa->site ?? '#' }}" target="_blank"
                                                    class="text-lime-500 hover:text-lime-400 truncate max-w-[250px] block">
                                                    {{ $empresa->site ?? 'N/A' }}
                                                </a>
                                            </dd>
                                        </div>
                                        <div class="flex items-center justify-between">
                                            ...
                                            <dd class="text-sm text-gray-300">{{ $empresa->created_at->format('d/m/Y') }}
                                            </dd>
                                        </div>
                                    </dl>
                                </div>
                            ...
                            <div class="bg-slate-900/50 px-6 py-4 flex items-center justify-end space-x-3">
                                ...
                                <button type="button"
                                    onclick="confirmarExclusaoEmpresa('{{ $empresa->id }}', '{{ addslashes($empresa->user->name) }}', '{{ $empresa->user->email }}', '{{ $empresa->cnpj_formatado ?? 'N/A' }}')"
    ...
    <!-- Script SweetAlert para exclusão de empresas -->
    <script>
        // Função para confirmar exclusão de empresa
        function confirmarExclusaoEmpresa(empresaId, empresaNome, empresaEmail, empresaCnpj) {
            Swal.fire({
                html: `
            <div class="text-center">
                ...
                <h3 class="text-xl mb-6 font-bold text-white mb-2">Excluir Empresa</h3>
                
                <p class="text-gray-300 mb-6 mt-6">Tem certeza que deseja excluir a empresa <strong class="text-white">"${empresaNome}"</strong>?</p>
            </div>
        `,
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/empresas/delete/${empresaId}`;
