...
@section('content')
    <div class="bg-slate-900 text-gray-200 min-h-screen">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="mb-8 text-center">
                <h1 class="text-3xl sm:text-4xl font-bold tracking-tight text-white">Meus Anúncios</h1>
                <p class="mt-1 text-md text-gray-400">Gerencie aqui todos os anúncios que sua empresa publicou.</p>
            </div>

            <!-- Barra de Busca -->
            <div class="mb-8 p-6 bg-gray-800/50 rounded-xl shadow-lg border border-gray-700">
                <form action="{{ route('advertisements.my') }}" method="GET">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-x-6 gap-y-4 items-end">
                        ...
                        <div class="md:col-span-7">
                            ...
                            <div class="relative">
                                ...
                                <a href="{{ route('advertisements.my', ['fields' => request('fields')]) }}"
            ...
            @if ($advertisements->isEmpty())
                <div class="text-center py-16 px-4 bg-gray-800/50 rounded-lg shadow-md border border-gray-700">
                    ...
                    <p class="mt-1 text-sm text-gray-400">Não encontramos anúncios seus que correspondam à busca.</p>
                    <div class="mt-6">
                        <a href="{{ route('advertisements.create') }}"
                            ...
                            <span class="text-base font-medium">Criar Novo Anúncio</span>
                        </a>
                    </div>
                </div>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    @foreach ($advertisements as $advertisement)
                        <div
                            ...
                            <a href="{{ route('advertisements.show', $advertisement->id) }}" target="_blank"
                            ...
                            <div class="p-6 flex-grow flex flex-col">
                                <div class="flex-grow">
                                    ...
                                    <h3 class="mt-1 text-lg font-bold text-white truncate">{{ $advertisement->titulo }}
                                    </h3>
                                    <p class="mt-1 text-sm text-gray-400 h-10 overflow-hidden">
                                        {{ $advertisement->subtitulo }}</p>
                                </div>
                                <div class="mt-4 border-t border-gray-700 pt-4 text-xs text-gray-500">Publicado em
                                    {{ $advertisement->created_at->format('d/m/Y') }}</div>
                            </div>
                            <div class="bg-slate-900/50 px-6 py-4 flex items-center justify-end space-x-3">
                                <a href="{{ route('advertisements.show', $advertisement->id) }}"
                                    ...
                                    title="Visualizar Anúncio">Visualizar</a>
                                ...
                                <button type="button"
                                    onclick="confirmarExclusaoMeuAnuncio('{{ $advertisement->id }}', '{{ addslashes($advertisement->titulo) }}', '{{ $advertisement->tipo ?? 'Geral' }}', '{{ $advertisement->created_at->format('d/m/Y') }}')"
    ...
    <script>
        // Função para confirmar exclusão de anúncio (meus anúncios)
        function confirmarExclusaoMeuAnuncio(anuncioId, anuncioTitulo, anuncioTipo, dataPublicacao) {
            Swal.fire({
                html: `
            <div class="text-center">
                ...
                <p class="text-gray-300 mb-4 mt-6">Tem certeza que deseja excluir seu anúncio <strong class="text-white">"${anuncioTitulo}"</strong>?</p>
                <p class="text-orange-400 text-xs mb-4">Este anúncio será removido permanentemente da plataforma.</p>
            </div>
        `,
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/advertisements/delete/${anuncioId}`;
